...
1.2.1 针对上述实测问题调整了布局留白、小符号候选核验、纵向裁剪和标点合并；在同一张桌面截图上自动保留 α/θ、去除多余点号与重复冒号。序数词恢复为正文。核对页可以点击公式定位编辑，并同时对照截图。见[改动前后像素结果与边界](usability/2026-09-21/ocr-iteration/README.md)。
