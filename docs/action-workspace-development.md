# Action workspace development reference

Historical implementation and test details retained for compatibility work. For the current reading workflow, see the [developer guide](../slipstream/README.md).
...
Before full analysis can be enabled, Slipstream first checks model metadata and then asks the selected model to process a fixed, fictional compatibility message with built-in `action-brief.prompt.v4`. If the model proposes tasks, materials, or deadlines, Slipstream sends the same fictional source and those claims to the same model for one short review. The result then travels through the parser and `ActionBriefV1` validator and must pass additional probe-specific checks for grounded translation, deadline, material, actions, terms, and process context; merely valid JSON is insufficient. The candidate contract and schema remain v1-compatible. This test never uses screenshots, clipboard content, task text, saved terms, or the optional custom analysis preference; cloud providers may record or charge for both requests. It is a representative compatibility probe, not a guarantee for arbitrary future inputs. Long tests show progress and can be cancelled without changing the saved configuration.
