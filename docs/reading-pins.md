...
3. 点击术语，展开“概念是什么”和“放在这段里”。解释标明模型判断的依据类型；若能在本段精确找到相关英文，读者可展开原句核对。译文和解释都能回到英文核对。
...
同一术语在完全相同的原文中再次查询时，先读取已保存的概念卡片，显示“已存卡片 · 本地解释”，无需再次请求模型。“打开卡片”可继续编辑或查看个人笔记；下次查询读取最新修改。不同原文不会自动套用旧解释。大小写和数学 Unicode 字形保留，因此同一段里的 `X`、`x`、`𝑥` 可以分别保存。此行为已通过隔离存储和隐藏窗口检查，当前待安装复验。
...
- 多个候选额外经过同一服务的一次短复核，只能删除，不新增或改写候选；译文先显示并支持完整复制与手动查询。复核限时 12 秒、不重试，失败保留译文并提示手动选词；零个或一个候选不额外请求。
- 同一张阅读卡片只展示一次英文与中文名称都相同的推荐概念，同词不同中文名称保留各自入口；后续段落仍可展开原文手动查询。
...
- 小卡片展开解释时自动增加空间，收起后恢复；读者手动调整的尺寸和位置被保留。字号同时调整解释和本段用法，切换概念从开头显示，收起解释将焦点返回术语按钮。
- 术语解释在点击后查询；选词位置由主进程验证。切换词句会取消前一次查询；先检查同原文的本地概念卡片，再使用当前阅读缓存或请求模型。
- 模型需区分原文明确给出的定义、仅由本段用法推断的解释和通用释义。展示“原文定义”前还要检查返回的英文句子确实出现在本段、包含所选术语，且术语旁有 `is defined as`、`denotes`、`:=` 等明确的定义表述；单独的 `is a` 可能只是描述假设，不足以确认定义。不满足时降为上下文解释或未核实的模型解释，原句仍可按需展开。这个校验减少错误的来源归属，不能证明中文释义本身正确；保存前仍应对照原文。
...
### 术语复核与连续阅读（2026-09-21）

当前实现已针对真实服务、原生窗口和保存重开做过本机检查。选词检查包含 18 段自拟文字和 4 段署名论文短摘录，各重复 3 次；不读取屏幕或用户材料。第三方论文的原始界面证据保留在本机。
