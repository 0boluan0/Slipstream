...
function analyzeShortcutAccelerator(value) {
  ...
  if (!isFunctionKey && !hasPrimaryModifier) {
    return { ok: false, reason: 'unsafe-unmodified' };
  }
...
function displayShortcutAccelerator(value) {
  return shortcutDisplayParts(value).join('+');
}

function sameShortcutAccelerator(left, right) {
