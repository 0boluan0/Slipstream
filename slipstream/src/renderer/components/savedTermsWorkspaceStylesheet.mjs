const SAVED_TERMS_STYLESHEET_FILE = 'SavedTermsLibrary.css';
const SAVED_TERMS_STYLESHEET_TIMEOUT_MS = 10000;
const MAX_SAVED_TERMS_STYLESHEET_ATTEMPT = 1;
const SAVED_TERMS_STYLESHEET_SELECTOR = 'link[data-workspace-stylesheet="saved-terms"]';
...
function parseAttempt(value) {
  ...
  if (!/^(?:0|1)$/u.test(value)) {
    throw new TypeError('A bounded Saved Terms stylesheet attempt is required');
  }
  return Number(value);
}
...
export function getSavedTermsStylesheetAttempt(moduleUrl) {
  if (typeof moduleUrl !== 'string' || moduleUrl.length === 0) {
    throw new TypeError('A Saved Terms module URL is required');
  }
  const parsed = new URL(moduleUrl);
  return parseAttempt(parsed.searchParams.get('workspace-attempt'));
}

export function buildSavedTermsStylesheetAttemptUrl(href, attempt, baseUrl) {
  if (
    typeof href !== 'string'
    || href.length === 0
    || !Number.isSafeInteger(attempt)
    || attempt < 0
    || attempt > MAX_SAVED_TERMS_STYLESHEET_ATTEMPT
    || typeof baseUrl !== 'string'
    || baseUrl.length === 0
  ) {
    throw new TypeError('A safe Saved Terms stylesheet URL and bounded attempt are required');
  }
  ...
  if (
    !sameBoundary
    || !stylesheet.pathname.endsWith(`/${SAVED_TERMS_STYLESHEET_FILE}`)
  ) {
    throw new TypeError('The Saved Terms stylesheet must stay inside the renderer boundary');
  }
...
function findReusableStylesheet(documentRef, requestedHref, attempt) {
  const candidates = [...documentRef.head.querySelectorAll(SAVED_TERMS_STYLESHEET_SELECTOR)];
...
export function loadSavedTermsWorkspaceStylesheet({
  attempt,
  documentRef = globalThis.document,
  href,
  timeoutMs = SAVED_TERMS_STYLESHEET_TIMEOUT_MS,
} = {}) {
  if (
    !documentRef?.head
  ...
  let requestedHref;
  try {
    requestedHref = buildSavedTermsStylesheetAttemptUrl(href, attempt, documentRef.baseURI);
  } catch (error) {
  ...
  link.dataset.workspaceStylesheet = 'saved-terms';
  ...
  const record = {
    ...
    cancel() {
      ...
      link.onerror = null;
      link.remove();
      rejectAttempt(new Error('Saved Terms stylesheet attempt was superseded'));
    },
  };
  ...
  link.onerror = () => rejectCurrentAttempt('Saved Terms stylesheet failed to load');
  timeoutId = timerHost.setTimeout(() => {
    rejectCurrentAttempt('Saved Terms stylesheet load timed out');
  }, timeoutMs);
