const SETTINGS_STYLESHEET_FILE = 'SettingsPanel.css';
const SETTINGS_STYLESHEET_TIMEOUT_MS = 10000;
const MAX_SETTINGS_STYLESHEET_ATTEMPT = 1;
const SETTINGS_STYLESHEET_SELECTOR = 'link[data-workspace-stylesheet="settings"]';
...
function parseAttempt(value) {
  ...
  if (!/^(?:0|1)$/u.test(value)) {
    throw new TypeError('A bounded Settings stylesheet attempt is required');
  }
  return Number(value);
}
...
export function getSettingsStylesheetAttempt(moduleUrl) {
  if (typeof moduleUrl !== 'string' || moduleUrl.length === 0) {
    throw new TypeError('A Settings module URL is required');
  }
  const parsed = new URL(moduleUrl);
  return parseAttempt(parsed.searchParams.get('workspace-attempt'));
}

export function buildSettingsStylesheetAttemptUrl(href, attempt, baseUrl) {
  if (
    typeof href !== 'string'
    || href.length === 0
    || !Number.isSafeInteger(attempt)
    || attempt < 0
    || attempt > MAX_SETTINGS_STYLESHEET_ATTEMPT
    || typeof baseUrl !== 'string'
    || baseUrl.length === 0
  ) {
    throw new TypeError('A safe Settings stylesheet URL and bounded attempt are required');
  }
  ...
  if (
    !sameBoundary
    || !stylesheet.pathname.endsWith(`/${SETTINGS_STYLESHEET_FILE}`)
  ) {
    throw new TypeError('The Settings stylesheet must stay inside the renderer boundary');
  }
...
function findReusableStylesheet(documentRef, requestedHref, attempt) {
  const candidates = [...documentRef.head.querySelectorAll(SETTINGS_STYLESHEET_SELECTOR)];
...
export function loadSettingsWorkspaceStylesheet({
  attempt,
  documentRef = globalThis.document,
  href,
  timeoutMs = SETTINGS_STYLESHEET_TIMEOUT_MS,
} = {}) {
  if (
    !documentRef?.head
  ...
  let requestedHref;
  try {
    requestedHref = buildSettingsStylesheetAttemptUrl(href, attempt, documentRef.baseURI);
  } catch (error) {
  ...
  link.dataset.workspaceStylesheet = 'settings';
  ...
  const record = {
    ...
    cancel() {
      ...
      link.onerror = null;
      link.remove();
      rejectAttempt(new Error('Settings stylesheet attempt was superseded'));
    },
  };
  ...
  link.onerror = () => rejectCurrentAttempt('Settings stylesheet failed to load');
  timeoutId = timerHost.setTimeout(() => {
    rejectCurrentAttempt('Settings stylesheet load timed out');
  }, timeoutMs);
