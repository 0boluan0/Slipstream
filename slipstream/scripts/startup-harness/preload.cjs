...
if (!SCENARIOS.has(scenario)) {
  throw new Error('renderer harness scenario is invalid');
}
...
async function invoke(channel, ...args) {
  ...
  switch (channel) {
    ...
    default:
      ...
      throw new Error('renderer harness adapter rejected a capability request');
  }

  markUnexpected(channel, 'invalid-arguments');
  throw new Error('renderer harness adapter rejected invalid arguments');
}
...
function on(channel, callback) {
  ...
  // The harness deliberately has no event source. Production renderer code can
  // subscribe to its fixed contract, but no native capability is connected.
  return () => {};
}
...
contextBridge.exposeInMainWorld('slipstreamRendererHarness', Object.freeze({
  protocol: PROTOCOL,
  getSummary: () => ({
    scenario,
