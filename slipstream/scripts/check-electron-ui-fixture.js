...
function checkNativeElectronBinding() {
  ...
  assert.match(mainSource, /app\.on\('ready', \(\) => \{\s*const storageStatus = store\.initializeStore\(\);\s*const launchedAtLogin = getOpenAtLoginState\(\{ app, supported: openAtLoginSupported \}\)\s*\.wasOpenedAtLogin;\s*wasOpenedAtLogin = launchedAtLogin;\s*if \(storageStatus\.state === 'ready'\) syncOpenAtLoginPreference\(\);\s*const settings = getStartupSettings\(\);\s*installAboutPanel\(\);\s*installApplicationMenu\(\);\s*registerAppQuitIpcHandlers\(\);\s*registerAppSettingsIpcHandlers\(\);\s*if \(!uiFixtureMode\.enabled\) registerIpcHandlers\(\);\s*createMainWindow\(settings\);\s*if \(uiFixtureMode\.enabled\) return;/s);
